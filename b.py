def ai_number_guesser():
    ...
    while low <= high:
        ...
        print(f"\n🎯 My guess is: {guess}")
        feedback = input(
            "Press (h) to guess higher 🔼, (l) to guess lower 🔽, or (c) for Correct ✅? "
        ).lower()

        if feedback == "c":
            print(
                f"🎉 Yay! I guessed your number {guess} correctly in {attempts} attempts, {name}! 🏆"
            )
            break
        elif feedback == "h":
            low = guess + 1
            ...
            print(
                "❗ Please enter 'h' for higher 🔼, 'l' for lower 🔽, or 'c' for correct ✅."
            )

    if low > high:
        print(
            f"🤔 Hmm, something went wrong, {name}. Are you sure you followed the rules?"
        )
